...
function displayType(){
	...
	// zuerst 10 Antworten gegeben, bevor ausgewertet werden kann
	if($questions.find("input:radio:checked").length == 10) {
		...
		var summe = parseFloat(farbe) + parseFloat(sommersprossen) + parseFloat(gesicht) + 
			parseFloat(zeit) + parseFloat(reaktion) + parseFloat(sonnenbrand) + 
			parseFloat(braun) + parseFloat(entwicklung)
